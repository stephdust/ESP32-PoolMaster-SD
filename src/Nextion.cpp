...
void InitTFT()
{
  ...
  myNex.writeNum(F("page1.vaR0.val"), 0); // precision : light
  myNex.writeNum(F("page1.vaR1.val"), 0); // precision : relay ou electrolyser
  myNex.writeNum(F("page1.vaR2.val"), 0); // precision : winter
  myNex.writeStr(F("page3.vaMCFW.txt"), FIRMW);
  myNex.writeStr(F("page3.vaTFTFW.txt"), TFT_FIRMW);
#ifdef ELECTROLYSE
  myNex.writeNum(F("storage.nElectrolyse.val"), 1);
#else
  myNex.writeNum(F("storage.nElectrolyse.val"), 0);
#endif
...
void UpdateTFT()
{
  ...
  if (TFT_ON)
  {
    ...
    if (CurrentPage == 1)
    {
      TFT_Filt = myNex.readNumber("page1.vaFilt.val") % 2; // ajout
      // if(TFT_Filt != FiltrationPump.IsRunning())  //modifier
      if (TFT_Filt != storage.FiltrationOn)
      {
        myNex.writeNum(F("page1.vaFilt.val"), storage.FiltrationOn ? 0 : 1);
      }

      TFT_Robot = myNex.readNumber("page1.vaRobot.val") % 2; // ajout
      // if(TFT_Robot != RobotPump.IsRunning())
      if (TFT_Robot != storage.RobotOn)
      {
        myNex.writeNum(F("page1.vaRobot.val"), storage.RobotOn ? 0 : 1);
        // TFT_Robot = !TFT_Robot;
      }

      TFT_R0 = myNex.readNumber("page1.vaR0.val") % 2; // ajout
      // if(digitalRead(RELAY_R0) != TFT_R0)
      if (digitalRead(RELAY_R0) != storage.LightOn)
      {
        myNex.writeNum(F("page1.vaR0.val"), storage.LightOn ? 1 : 0);
      }

      TFT_R1 = myNex.readNumber("page1.vaR1.val") % 2;
#ifdef ELECTROLYSE // ajout
      if (digitalRead(ORP_PROD) != storage.ElectrolyseOn)
      {
        myNex.writeNum(F("page1.vaR1.val"), storage.ElectrolyseOn ? 1 : 0);
      }
#else
...
// ajout
// Electrolyser button was toggled
// printh 23 02 54 09
#ifdef ELECTROLYSE
void trigger9()
{
  char Cmd[100] = "{\"Electro\":1";
  if (digitalRead(ORP_PROD))
    Cmd[11] = '1';
  xQueueSendToBack(queueIn, &Cmd, 0);
  LastAction = millis();
}
