...
AddonStruct myWaterMeterPulse = {0};
...
void WaterMeterPulseSaveMeasures(void *pvParameters)
{
    ...
    AddonsPublishTopic(myWaterMeterPulse.MQTTTopicMeasures, root);
}
...
void WaterMeterPulseLoadMeasures(void *pvParameters)
{
    ...
    StaticJsonDocument<capacity> root;
    if (!AddonsReadRetainedTopic(myWaterMeterPulse.MQTTTopicMeasures, root)) 
        return;
...
AddonStruct WaterMeterPulseInit(void)
{
    ...
    myWaterMeterPulse.MQTTTopicMeasures = AddonsCreateMQTTTopic(myWaterMeterPulse.name, "Meas");
